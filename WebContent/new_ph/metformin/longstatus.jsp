...
<div class="row">
	<div class="col-12 viz-header-section">
		<h2 id="${param.block}longstatus-title" class="viz-title"></h2>
		<div class="btn-group float-right">
			<button type="button" class="btn btn-sm btn-light dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
				<i class="fas fa-download"></i>
			</button>
			<div class="dropdown-menu dropdown-menu-right">
				<a class="dropdown-item" onclick="${param.block}save_viz_pass_longstatus('.jpg');">Save as JPG</a>
				<a class="dropdown-item" onclick="${param.block}save_viz_pass_longstatus('.jpg');">Save as PNG</a>
				<a class="dropdown-item" onclick="${param.block}save_viz_pass_longstatus('.jpg');">Save as SVG</a>
			</div>
		</div>
	</div>
	<div class="col-12">
...
function ${param.block}save_viz_pass_longstatus(extension){
	...
	var text = '';
	if (mode =='pie'){		
		text = "Long COVID Status Percentages of ${param.topic_title}" + extension;
	} else if (mode == 'bar'){
		text = "Counts of ${param.topic_title} by Long COVID Status" + extension;
	} else {
		text = "Long COVID Status Percentages of ${param.topic_title}" + extension;
	};
	
	saveVisualization('${param.block}_longstatus_viz', text);
};
...
if (title_mode =='pie'){		
	var title = "Long COVID Status Percentages of ${param.topic_title}";
	$("#${param.block}longstatus-title").text(title);
} else if (title_mode == 'bar'){
	var title = "Counts of ${param.topic_title} by Long COVID Status";
	$("#${param.block}longstatus-title").text(title);
} else {
	var title = "Long COVID Status Percentages of ${param.topic_title}";
	$("#${param.block}longstatus-title").text(title);
...
function ${param.block}_longstatus_refresh() {
	...
	d3.select("#${param.block}_longstatus_viz").select("svg").remove();
	
	if (mode =='pie'){		
		localPieChart_new(${param.block}_LongstatusArray, properties);
	} else if (mode == 'bar'){
		localHorizontalBarChart_new(${param.block}_LongstatusArray, properties);
	} else {
