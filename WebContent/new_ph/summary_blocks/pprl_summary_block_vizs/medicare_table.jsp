...
$.getJSON("<util:applicationRoot/>/new_ph/summary_blocks/pprl_summary_block_vizs/medicare_table_feed.jsp", function(data, error){
    	    ...
    	    buttons: [{
              ...
              action: function ( ) {
            	  domtoimage.toBlob(document.getElementById('${param.target_div}-table'))
            	    .then(function (blob) {
            	        window.saveAs(blob, 'Medicare_Enhancement.png');
            	    });
              }
    	    }, {
    	      text: 'JPG',
              ...
              action: function ( ) {
            	  domtoimage.toJpeg(document.getElementById('${param.target_div}-table'), { quality: 1, bgcolor: 'white' })
            	    .then(function (dataUrl) {
            	        ...
            	        link.download = 'Medicare_Enhancement.jpg';
            	        link.href = dataUrl;
            	        link.click();
            	    });
             }
    	...
    	order: [[9, 'desc']],
     	columns: [
        	...
        	{ data: 'common_cnt', visible: false, orderable: true,className: 'export'  },
     		{ data: 'additional_in_cms', visible: true, orderable: true, orderData: [5], className: 'medicaretext', width: '240px', 
     			render: function(data, type, row, meta){
	 				var large_div = $("<div></div>");
	 				var text_div = $("<div></div>", {"class": "viz_text"});
	
	 				text_div.append(function(){
	                    return $("<div><strong>" +  row['additional_in_cms'] + "</strong><span class='smalltext'> : (Medicare Total: "
     				...
     				viz_div.append(function(){
						...
						for (const [key, value] of Object.entries(row)) {
							...
							if (key == 'additional_in_cms'){
								var width = (value/max)*100
								bars.push($("<div></div>",{"class": "bar " + "barmedicare" }).css({ "width": width + "%"}))
							}                            
                        }
                        return bars;
					});
