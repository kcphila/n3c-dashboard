...
<sql:query var="ages" dataSource="jdbc/N3CPublic">
	select jsonb_pretty(jsonb_agg(done))
		from (select
			...
			total_avg,
			(select max(total_avg) from n3c_dashboard_ph.cms_avg_counts_all where cms_type = 'Medicaid') as max_total_avg,
			round(percentage_increase::numeric,2) as percentage_increase
		  from n3c_dashboard_ph.cms_avg_counts_all
		  where cms_type = 'Medicaid'
	) as done;
</sql:query>
{
    "headers": [
        ...
        {"value":"additional_in_cms", "label":"<i class='fas fa-square'>&nbsp;</i> Avg Medicaid Enhancement"},
        {"value":"additional_in_cms_download", "label":"Avg Medicaid Enhancement"},
