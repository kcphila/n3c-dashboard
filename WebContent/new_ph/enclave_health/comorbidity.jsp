...
<div class="row">
	<div class="col-12 viz-header-section">
		<div style="display:flex; flex-wrap: nowrap;">
			<h2 id="${param.block}comorbidity-title" class="viz-title"></h2>
			<div>
				<button type="button" class="btn btn-sm btn-light dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
					<i class="fas fa-download"></i>
				</button>
				<div class="dropdown-menu dropdown-menu-right">
					<a class="dropdown-item" onclick="save_viz_pass_comorbidity('.jpg');">Save as JPG</a>
					<a class="dropdown-item" onclick="save_viz_pass_comorbidity('.png');">Save as PNG</a>
					<a class="dropdown-item" onclick="save_viz_pass_comorbidity('.svg');">Save as SVG</a>
				</div>
			</div>
		</div>
	</div>
...
function save_viz_pass_comorbidity(extension){
	...
	var text = '';
	if (mode =='pie'){		
		text = "Comorbidity Percentages of ${param.topic_title}" + extension;
	} else if (mode == 'bar'){
		text = "Counts of ${param.topic_title} by Comorbidity" + extension;
	} else {
		text = "Comorbidity Percentages of ${param.topic_title}" + extension;
	};
	
	saveVisualization('${param.block}_comorbidity_viz', text);
};
...
if (title_mode =='pie'){		
	var title = "Comorbidity Percentages of ${param.topic_title}";
	$("#${param.block}comorbidity-title").text(title);
} else if (title_mode == 'bar'){
	var title = "Counts of ${param.topic_title} by Comorbidity";
	$("#${param.block}comorbidity-title").text(title);
} else {
	var title = "Comorbidity Percentages of ${param.topic_title}";
	$("#${param.block}comorbidity-title").text(title);
...
function ${param.block}_comorbidity_refresh() {
	...
	if (mode =='pie'){		
		localPieChart_new(${param.block}_ComorbidityArray, properties);
	} else if (mode == 'bar'){
		localHorizontalBarChart_new(${param.block}_ComorbidityArray, properties);
	} else {
		localPercentageBarChart_new(${param.block}_ComorbidityArray, properties);
	};
