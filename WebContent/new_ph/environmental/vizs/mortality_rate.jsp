...
var barLabelWidth = 240; // padding between bar and bar labels (left)
	...
	function ${param.block}draw_bar_rate() {
		...
		barsContainer.selectAll("rect").data(${param.block}data).enter().append("rect")
			...
			.on('mousemove', function(d){
				...
				.style("left", (d3.event.pageX + 5) + "px")
				.style("top", (d3.event.pageY - 28) + "px")
				.html("<strong>" + d.label + "</strong><br><strong>Total Patients:</strong> " + d.total.toLocaleString() + "<br><strong>Total " + d.variable + ":</strong> " + d.part.toLocaleString() + "<br><strong>Percent:</strong> " + d.patient_display.toLocaleString() +"%");
			})
			.on('mouseout', function(d){
				 d3.selectAll(".tooltip").remove(); 
			});
