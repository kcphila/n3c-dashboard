				...
				<div id="${param.block}age_panel">
					...
					<select id="${param.block}-age-select" multiple="multiple">
						<sql:query var="cases" dataSource="jdbc/N3CPublic">
							select age, age_abbrev from n3c_dashboard.age_map_sec order by age_seq;
						</sql:query>
...
<script>
	function checkPeds() {
		$('input[type="checkbox"][value="0-17"]').prop('checked',true).trigger('change');
	...
	function checkAdult() {
		$('input[type="checkbox"][value="0-17"]').prop('checked',false).trigger('change');
		$('input[type="checkbox"][value="Unknown"]').prop('checked', false).trigger('change');
