...
<graph:graph>
    <sql:query var="projects" dataSource="jdbc/N3CPublic" >
    	...
    	from n3c_dashboard.alcohol_map join n3c_dashboard_ph.substance_alc_smo_combined on (alcohol_condition = secondary)
    ...
    <sql:query var="persons" dataSource="jdbc/N3CPublic" >
    	...
    	from n3c_dashboard.smoking_map join n3c_dashboard_ph.substance_alc_smo_combined on (secondary = smoking_status)
    	group by 1,2 order by 1;
    </sql:query>
    ...
    <sql:query var="edges" dataSource="jdbc/N3CPublic">
         select alcohol_map.secondary_seqnum as alcohol_condition_seqnum,smoking_map.secondary_seqnum,
    ...
    <c:forEach items="${edges.rows}" var="row">
    	<c:choose>
    		<c:when test="${not empty param.covid_count}">
		      <graph:edge source="alcohol_${row.alcohol_condition_seqnum}" target="smoking_${row.secondary_seqnum}"  weight="${row.covid_weight}" auxInt="${row.covid_count}" />
      		</c:when>
    		<c:when test="${not empty param.all_count}">
		      <graph:edge source="alcohol_${row.alcohol_condition_seqnum}" target="smoking_${row.secondary_seqnum}"  weight="${row.all_weight}" auxInt="${row.all_count}" />
    		</c:when>
    		<c:otherwise>
		      <graph:edge source="alcohol_${row.alcohol_condition_seqnum}" target="smoking_${row.secondary_seqnum}"  weight="${row.all_weight}" auxInt="${row.all_count}" />
    		</c:otherwise>
