...
<div class="row">
	...
	<div class="col-12">
		...
		<div id="${param.block}_smoking_viz" class="dash_viz"></div>
	</div>
</div>
...
function save_viz_pass_smoking(extension){
...
function ${param.block}_smoking_refresh() {
	...
	d3.select("#${param.block}_smoking_viz").select("svg").remove();
	
	if (mode =='pie'){		
		localPieChart_new(${param.block}_SmokingArray, properties);
	} else if (mode == 'bar'){
		localHorizontalBarChart_new(${param.block}_SmokingArray, properties);
	} else {
		localPercentageBarChart_new(${param.block}_SmokingArray, properties);
	};
