...
<div class="row">
	...
	<div class="col-12">
		<div id="${param.block}_alcoholstatus_viz" class="dash_viz"></div>
	</div>
</div>
...
function save_viz_pass_alcoholstatus(extension){
...
function ${param.block}_alcoholstatus_refresh() {
	var properties = {
	...
	d3.select("#${param.block}_alcoholstatus_viz").select("svg").remove();
	
	if (mode =='pie'){		
		localPieChart_new(${param.block}_AlcoholStatusArray, properties);
	} else if (mode == 'bar'){
		localHorizontalBarChart_new(${param.block}_AlcoholStatusArray, properties);
	} else {
		localPercentageBarChart_new(${param.block}_AlcoholStatusArray, properties);
	};
