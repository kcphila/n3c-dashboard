...
async function county_init() {
	...
	const fetchPromise2 = fetch(getApplicationRoot() + "/new_ph/environmental/feeds/geospatial_county.jsp");
...
function county_draw() {
	...
	site_by_county = d3.map(site_data, function(d) { return d.code+'|'+d.countyname; });
	...
	county_g.selectAll(".county")
		...
		.html(function(d) {
			...
			if (site == undefined)
				...
				return site.countyname + ", " + site.statename
					+ "<br><b>COVID- Patients</b>: " + site.non_count
					+ "<br><b>COVID+ Patients</b>: " + site.covid_count
					+ "<br><b>Mortality</b>: " + site.patient_count_died
					+ "<br><b>Mortality due to COVID</b>: " + site.patient_count_died_cause_covid
					;
		});
}
...
d3.select("#county_node_selector").node().addEventListener("change", function(d) { county_order(d3.select("#county_node_selector").node().value); });

function county_order(value) {
	console.log(value);
	county_opacityScale = d3.scaleLinear()
		.domain([0, d3.max(site_data, function(d) { return Math.sqrt(nodeValue(d)); })])
		.range([0.50, 1.0]);
