...
function OddsRatioGroupedChart(data1, properties) {
	...
	var g = chart.append("g")
		...
		var bands = chart.selectAll(".bandlines")
			.data(groupedData)
			.enter()
			.append("g");
		
		var bandEntries = bands.selectAll("bandentries")
			.data(function(d,i) {
				d.values[0].primary = i;
				d.values[1].primary = i;
				d.values[2].primary = i;
				return d.values;})
			.enter()
			.append("g");
		...
		bandEntries
			.selectAll("vertLines")
			.data(function(d,i) {
				d.values[0].primary = d.primary;
				d.values[0].secondary = i % d.primary;
				return d.values;})
			.enter()
			.append("line")
			...
			.attr("y1", function(d) {
				switch (d.secondary) {
					case 0:
						return (y(d.primary) + y.bandwidth() / 4 - 3*whiskerGap);
					case 1:
						return (y(d.primary) + y.bandwidth() / 2 - whiskerGap);
					case 2:
						return (y(d.primary) + (3 * y.bandwidth()) / 4 + whiskerGap);
					}
				})
			.attr("y2", function(d) {
				switch (d.secondary) {
		...
		bandEntries
			...
			.attr("y1", function(d) {
				switch (d.secondary) {
					case 0:
						return (y(d.primary) + y.bandwidth() / 4 - 3*whiskerGap - properties.symbolSize * 2 );
					case 1:
						return (y(d.primary) + y.bandwidth() / 2 - whiskerGap - properties.symbolSize * 2 );
					case 2:
						return (y(d.primary) + (3 * y.bandwidth()) / 4 + whiskerGap - properties.symbolSize * 2 );
					}
				})
			.attr("y2", function(d) {
				switch (d.secondary) {
			...
			case "hazard":
				bandEntries
					...
					.attr("y", function(d) {
						switch (d.secondary) {
							case 0:
								return (y(d.primary) + y.bandwidth() / 4 - 3*whiskerGap - properties.symbolSize );
							case 1:
								return (y(d.primary) + y.bandwidth() / 2 - whiskerGap - properties.symbolSize );
							case 2:
								return (y(d.primary) + (3 * y.bandwidth()) / 4 + whiskerGap - properties.symbolSize );
							}
						})
