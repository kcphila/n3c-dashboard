...
<%@ taglib prefix="util" uri="http://icts.uiowa.edu/tagUtil"%>
	...
	<div class="col col-12 col-lg-2 side-col">
		...
		<div id="filter_checks" class="panel-primary filter-section filter_checks">
			<div class="filters-label">
				<h4 style="flex-fill:1;">Filters</h4>
				<div>
					<span class="btn btn-link p-0 filters-pin" data-toggle="tooltip" data-placement="top" title="" data-original-title="Pin/Unpin Filters Section" aria-hidden="true" onclick="sticky_toggle()"></span>
				</div>
			</div>
						...
						<div id="comorbidities_body" class="collapse" aria-labelledby="comorbidities_heading" data-parent="#comorbidities_accordion">
							<div class="card-body">
								<div id="comorbidities_panel">
									<small>Note: Select a single option to see all patients with that comorbidity. Each additional selection will only show patients with that specific combination.</small><br>
									...
									<sql:query var="comorbidities" dataSource="jdbc/N3CPublic">
										select distinct(comorbidity_name) from n3c_dashboard_ph.demo_ungrouped_cci_cov_csd where comorbidity_name is not null order by comorbidity_name;
									</sql:query>
									<c:forEach items="${comorbidities.rows}" var="row" varStatus="rowCounter">
										<label class="filter_option"><input type="checkbox" name="comorbidity" value="${row.comorbidity_name}" > ${row.comorbidity_name}</label>
									</c:forEach>
								</div>
							</div>
						</div>
							...
							<div class="card-body">
								<div id="status_panel">
									...
									<c:forEach items="${status.rows}" var="row" varStatus="rowCounter">
										<label class="filter_option"><input type="checkbox" name="covid" value="${row.status_abbrev}" > ${row.status_abbrev}</label>
									</c:forEach>
								</div>
							</div>
						</div>
			...
			<c:if test="${param.long_covid_status}">
				<div class="panel-body">
							...
							<div class="card-body">
								<div id="vaccinated_panel">
									...
									<c:forEach items="${vaccinated.rows}" var="row" varStatus="rowCounter">
										<label class="filter_option"><input class="vaccinationstatus" type="checkbox" name="vaccinated" value="${row.vaccinated_abbrev}" > ${row.vaccinated_abbrev}</label>
									</c:forEach>
								</div>
							</div>
						</div>
	...
	<script>
		...
		function sticky_toggle(){
			...
			$('#filter_checks span.btn').toggleClass( "stuck" );
		}
	</script>
